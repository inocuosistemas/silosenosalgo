...
export const onRequestGet: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  const rows = await env.DB.prepare(
    `SELECT m.user_id AS userId, u.username AS username, m.color AS color, m.bib AS bib,
            ...
            m.plan_overlay IS NOT NULL AS hasPlan, m.organizer AS organizer, m.retired_at AS retiredAt, m.retired_km AS retiredKm, m.manual_pasos AS manualPasos, m.oficial_meta_at AS oficialAt, u.sin_cuenta AS sinCuenta,
    ...
    joinedAt: number; lastSeen: number | null; hasPlan: number; sessionId: string | null; retiredAt: number | null; retiredKm: number | null; manualPasos: string | null; oficialAt: number | null; sessionUpdatedAt: number | null; sinCuenta: number
    organizer: number
  }>()
  ...
  const event: EventInfo = {
    ...
    betsEnabled: !!ev.betsEnabled,
    notes: ev.notes,
    planUpdatedAt: ev.planUpdatedAt,
