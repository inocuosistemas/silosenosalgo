...
export const onRequestGet: PagesFunction<Env> = async ({ params, env }) => {
  ...
  const row = await env.DB.prepare(
    `SELECT status, title, plan_share_id AS planShareId, started_at AS startedAt, expires_at AS expiresAt,
            ended_at AS endedAt, lat, lon, track_km AS trackKm, speed, heading, accuracy, altitude,
            fix_at AS fixAt, updated_at AS updatedAt, trail
  ...
  const body: TrackStateResponse = {
    status, title: row.title, startedAt: row.startedAt, expiresAt: row.expiresAt,
