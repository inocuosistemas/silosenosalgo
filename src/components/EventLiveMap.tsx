...
/*
 * Y pasado un rato más largo ya no es un hueco, es un agujero de cobertura: en
 * montaña lo normal es quedarse sin red un buen rato —un valle, un bosque, una
 * cara norte— y el punto se queda clavado donde entró. A los seis minutos basta
 * con apagarlo; pasado el plazo de SU baliza hay que DECIRLO, porque quien mira
 * lleva un rato viendo a alguien parado en el mismo sitio y la conclusión
 * natural —"le ha pasado algo"— es casi siempre la equivocada.
 *
 * Cuánto es "un rato" depende de cada baliza y lo dice ella: ver
 * `shared/cadencia.ts` y `plazosDe`.
 */
...
export default function EventLiveMap({ source, vista, onVista, nav }: {
  ...
  const rows = useMemo(() => {
    return (runners ?? []).map((r0) => {
      ...
      const margin = km !== null && cutoffs.length > 0 && r.status === 'active' && referencia !== null
        ...
        : null
      /**
       * Y cuánto silencio es normal EN SU BALIZA, no en general.
       *
       * Una que emite cada 500 m —el modo ahorro de las ultras— no manda nada
       * mientras su dueño está parado, y con los plazos de todos salía "sin
       * cobertura" cada rato estando perfecta. Ver `shared/cadencia.ts`; sin
       * cadencia declarada, los de siempre.
       */
      ...
      const stale = !modoManual && (r.status === 'ended'
      ...
      const lost = !modoManual && !enPausa && r.status === 'active' && r.updatedAt !== null && now - r.updatedAt > plazos.perdido
      ...
      const callado = !modoManual && !enPausa && r.status === 'active' && r.fix !== null && r.updatedAt !== null
      ...
      const ritmoMinKm = km != null && km > 1 && minutosEnCarrera != null && minutosEnCarrera > 0
        ? minutosEnCarrera / km
        : null
      const abandono: Abandono | null = idle || armed || retirado || modoManual
        ? null
        : detectaAbandono({
            pasos,
            ahoraMs: now,
  ...
  /** A quién le toca asomar su frase. Sale de `rows` y no de `parrilla` para
   *  que el cuadro de la espera y la tira del mapa vayan al mismo compás: son
   *  la misma gente, y dos relojes distintos se pisarían al cambiar de vista. */
  const clavesConFrase = useMemo(
    () => rows.filter(({ r }) => r.bocadillo).map(({ key }) => key),
    [rows],
  )
                ...
                <ul className="mt-1.5 max-h-44 space-y-1 overflow-y-auto scrollbar-fantasma pr-0.5 pt-10">
                  {parrilla.map(({ r, key, idle, armed, lost, retirado }) => (
                    <li key={key} className="flex items-center gap-1.5 text-[11px]">
                      ...
                      {/* El nombre, con su globo colgando. El envoltorio es el
    ...
    <div className="rounded-xl border border-slate-700 bg-slate-900/95 p-3 backdrop-blur">
      ...
      <div className="mt-2 grid grid-cols-3 gap-2 text-center">
        ...
        {congelado?.at != null
          ...
          : row.modoManual
          ? <Dato valor="✎" unidad="manual · sin pasos" tono="text-sky-300" />
          : <Dato
              valor={ago ?? '—'}
